...
//   private customOrderBook: CustomOrderBook = {};
  ...
  private async getOrderDetailsByOrderId(access_token: string, orderId: string, broker: "UPSTOCKS" | "ZERODHA") {
    let orderDetails;
    switch (broker) {
      case "UPSTOCKS":
        const upstoxBroker = UpstoxBroker.getInstance();
  ...
  private async placeOrderInBroker(access_token: string, orderDetails: OrderDetails, broker: "UPSTOCKS" | "ZERODHA"): Promise<string> {
    let order_id:string = "";
    try {
      switch (broker) {
        case "UPSTOCKS":
...
//   }
  ...
  public async exitSinglePosition(access_token: string, position: any, broker: "UPSTOCKS" | "ZERODHA", ltp: number): Promise<void> {
  ...
  public async getPositions(access_token: string, broker: "UPSTOCKS" | "ZERODHA"): Promise<any> {
